...
from .dataset import load_table, sha256
from .features import CATEGORICAL_INDICES, FEATURE_NAMES, SCHEMA, extract_features


def train(table: Path, output: Path, *, device="auto", n_estimators=1,
          fit_mode="fit_with_cache", seed=0, model_path=None):
    if output.exists():
    ...
    kwargs = dict(device=device, n_estimators=n_estimators, fit_mode=fit_mode,
                  random_state=seed, categorical_features_indices=list(CATEGORICAL_INDICES))
    ...
    manifest = {
        "schema": SCHEMA, "feature_names": list(FEATURE_NAMES), "model_version": "v3.5",
        "tabpfn_version": importlib.metadata.version("tabpfn"),
        "checkpoint_path": str(model.model_path),
        "context_sha256": sha256(table), "fitted_sha256": sha256(fitted_path),
        "context": metadata, "classes": [int(value) for value in model.classes_],
        "device_requested": device, "n_estimators": n_estimators,
        "fit_mode": fit_mode, "fit_seconds": fit_seconds,
...
class Policy:
    def __init__(self, directory: Path, device="auto"):
        ...
        if self.manifest["schema"] != SCHEMA or self.manifest["feature_names"] != list(FEATURE_NAMES):
        ...
        if installed != self.manifest["tabpfn_version"]:
            raise ValueError(f"Fitted policy requires tabpfn=={self.manifest['tabpfn_version']}; "
                             f"installed {installed}. Refit or use the original package version.")
        fitted = directory / "policy.tabpfn_fit"
        if sha256(fitted) != self.manifest["fitted_sha256"]:
            raise ValueError("Fitted artifact hash differs from its manifest")
    ...
    def predict_ram(self, ram):
        ...
        return {"action": action, "buttons": button_names(action),
...
def evaluate(policy: Policy, table: Path, batch_size=128):
    ...
    started = time.perf_counter()
    predictions = np.concatenate([policy.model.predict(X[i:i + batch_size])
                                  for i in range(0, len(X), batch_size)])
    return {"rows": len(y), "seconds": time.perf_counter() - started,
            "accuracy": float(accuracy_score(y, predictions)),
            "balanced_accuracy": float(balanced_accuracy_score(y, predictions)),
            "unseen_target_actions": sorted(set(map(int, y)) - set(policy.manifest["classes"])),

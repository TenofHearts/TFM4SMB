...
def parser():
    ...
    prep = sub.add_parser("prepare", help="Extract a sampled table from selected trajectory PNGs")
    ...
    prep.add_argument("--label-offset", type=int, choices=[0, 1], default=1,
                      help="1: RAM[f] -> action[f+1] (default); 0: same-frame labels")
    prep.add_argument("--ram-encoding", choices=["dataset-cr", "raw"], default="dataset-cr")
    fit = sub.add_parser("train", help="Fit pretrained v3.5 on the context (no gradient updates)")
    ...
    fit.add_argument("--fit-mode", choices=["fit_with_cache", "fit_preprocessors", "low_memory"], default="fit_with_cache")
    fit.add_argument("--seed", type=int, default=0)
    fit.add_argument("--model-path", type=Path, help="Optional local v3.5 safetensors checkpoint")
    predict = sub.add_parser("predict", help="Predict from a dataset PNG or raw 2048-byte RAM dump")
    ...
    predict.add_argument("--ram-encoding", choices=["dataset-cr", "raw"], default="dataset-cr")
    predict.add_argument("--device", default="auto")
    ev = sub.add_parser("evaluate", help="Evaluate on a separately prepared table you supply")
    ...
    doctor = sub.add_parser("doctor", help="Inspect compute and optionally exercise an emulator")
    doctor.add_argument("--env-id", help="Reset and step this environment; no model needed")
    commands = {"prepare": prep, "train": fit, "predict": predict, "evaluate": ev,
                "play": play, "doctor": doctor}
...
def parse_args(argv=None):
    ...
    required = {"prepare": ["data", "output"], "train": ["context", "output"],
                "predict": ["model"], "evaluate": ["model", "data"],
                "play": ["model", "output"], "doctor": []}
...
def concise(value):
    if isinstance(value, dict):
        return {key: (len(item) if key == "feature_names" else concise(item))
...
def main():
    ...
    if args.command == "prepare":
        ...
        result = prepare(args.data, args.output, outcome=args.outcome, stride=args.stride,
                         max_rows=args.max_rows, seed=args.seed, label_offset=args.label_offset,
                         encoding=args.ram_encoding)
    elif args.command == "train":
        from .policy import train
        result = train(args.context, args.output, device=args.device, n_estimators=args.n_estimators,
                       fit_mode=args.fit_mode, seed=args.seed, model_path=args.model_path)
        ...
        if args.command == "predict":
            ...
            ram = read_frame(parse_frame(args.png), args.ram_encoding) if args.png else np.frombuffer(args.ram.read_bytes(), dtype=np.uint8)
            result = policy.predict_ram(ram)
        elif args.command == "evaluate":
            result = evaluate(policy, args.data, args.batch_size)
        else:
            from .game import play
            result = play(policy, args.env_id, args.output, episodes=args.episodes,
                          max_frames=args.max_frames, action_repeat=args.action_repeat,
                          seed=args.seed, render=args.render)
    else:
        import torch
        result = {"torch": torch.__version__, "tabpfn": importlib.metadata.version("tabpfn"),
                  "cuda_available": torch.cuda.is_available(),
                  "gpus": [{"name": torch.cuda.get_device_name(i),
                            "vram_gib": torch.cuda.get_device_properties(i).total_memory / 1024**3}
        ...
        if args.env_id:
            ...
            try:
                ...
                step_env(env, 0)
                ram = get_ram(env)
                result["environment"] = {"id": args.env_id, "ram_bytes": len(ram),
